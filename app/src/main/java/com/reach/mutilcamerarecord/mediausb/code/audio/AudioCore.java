package com.reach.mutilcamerarecord.mediausb.code.audio;
...
import com.reach.mutilcamerarecord.mediausb.code.MediaCodecHelper;
...
import com.reach.mutilcamerarecord.mediausb.model.AudioBuff;
...
public class AudioCore {
    ...
    public void queueAudio(byte[] rawAudioFrame) {
        ...
        if (orignAudioBuffs[targetIndex].isReadyToFill) {
            //Log.d("queueAudio","queueAudio,accept ,targetIndex" + targetIndex);
            System.arraycopy(rawAudioFrame, 0, orignAudioBuffs[targetIndex].buff, 0, rawAudioFrame.length/*mediaMakerConfig.audioRecoderBufferSize*/);
            ...
            Log.d("queueAudio","queueAudio,abandon,targetIndex" + targetIndex);
        }
    }
    ...
    public boolean prepare(RecordConfig resConfig) {
        synchronized (syncOp) {
            ...
            mediaMakerConfig.mediacodecAACSampleRate = 16000;//44100;
            mediaMakerConfig.mediacodecAACChannelCount = 1;
            mediaMakerConfig.mediacodecAACBitRate = 64000;//32 * 1024;
            mediaMakerConfig.mediacodecAACMaxInputSize = 3200;//8820;
            ...
            for (int i = 0; i < audioQueueNum; i++) {
                orignAudioBuffs[i] = new AudioBuff(AudioFormat.ENCODING_PCM_16BIT, 640/*orignAudioBuffSize*/);
            }
            orignAudioBuff = new AudioBuff(AudioFormat.ENCODING_PCM_16BIT, 640/*orignAudioBuffSize*/);
